...
export default function Page(){
  ...
  return <main className={"app "+view}>
    ...
    {view==="placement"&&<section className="placementQuest">
      {placementPhase==="intro"&&<div className="placementIntro"><button className="placementClose" onClick={()=>placementComplete&&setView("home")} aria-label="Close">{placementComplete?"×":""}</button><div className="placementPortal"><Creature/></div><small className="eyebrow">PLACEMENT QUEST</small><h1>Find your starting trail</h1><p>A few quick word clues will help your Wordling choose the best place to begin. There are no scores—just clues.</p><div className="placementPromises"><span>◷ 3–5 minutes</span><span>✦ 8 word clues</span><span>♡ You can ask for help</span></div><button className="primary" onClick={beginPlacement}>Begin the quest →</button></div>}
      {placementPhase==="questions"&&placementCurrent&&<><div className="missionbar"><button onClick={()=>placementComplete?setView("parent"):setPlacementPhase("intro")}>×</button><div><i style={{width:(placementIndex+(placementFeedback?1:0))/8*100+"%"}}/></div><b>{placementIndex+1}/8</b></div><div className="stage placementStage"><div className="coach"><Creature small/>{placementCurrent.mode==="read"?"Say this word out loud.":placementCurrent.mode==="choice"?"Listen, then find the word.":"Spell the word you hear."}</div>
